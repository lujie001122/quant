...
从早期模块升级，提供持仓/订单/行情/信号状态的统一读写接口。
...
# ── 默认回退 ──────────────────────────────────────────────────────────
_DEFAULT_CODES = ["159516", "515880", "588170", "159532", "515050", "159611"]
_DEFAULT_NAMES = {
    "159516": "半导体设备ETF",
    "515880": "通信ETF",
    "588170": "科创半导体ETF",
...
def get_tracked_codes():
    ...
    codes = set()
    if pool:
        codes.update(pool.get("etf_pool", []))
    else:
...
def write_order(code, action, shares, price, contract, status='pending'):
    ...
    today = datetime.now().strftime('%Y%m%d')
    # 序号从该目录下已有文件数计算，避免同日同code同action第二次下单覆盖
    existing = len([f for f in os.listdir(_ORDERS_DIR) if f.startswith(f'{today}_{code}_{action}') and f.endswith('.json')])
    seq = existing + 1
    order_path = os.path.join(_ORDERS_DIR, f'{today}_{code}_{action}_{seq}.json')
    order = {
        'code': code,
        'action': action,
        'shares': shares,
