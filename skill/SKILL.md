...
- **Backtrader position management architecture**: user corrected "这不是共享资金池，这是仓位管理" — the strategy should NOT manage cash/position limits manually. Use backtrader's native `self.buy()/self.sell()` with size, and let the broker handle fund constraints. The `_buy()` method should take a percentage (e.g. 0.30 for 30% of total fund value) and compute target shares, then let backtrader's broker reject if insufficient funds. The `_sell()` method should take a percentage of current position. This is cleaner than the old approach of manually tracking cash and position limits.
