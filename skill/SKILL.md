...
**Backtest baseline after v3.3 consistency fixes (2026-08-01, backtrader engine, backtest-live consistency)**: 5yr +97.88%/8.88%dd/Calmar 1.65/Sharpe 1.21/40笔/52.5%wr/4.23pf. Four fixes applied: (1) entry ratio unified to total fund × pct (user chose total fund base over per-ETF base — "应该是按比例，不要写死"), (2) AO uses (H+L)/2 (was close approximation), (3) COOLDOWN_DAYS=2 unified in both files, (4) trend profit sell + MA5 active share sell added to backtest. An intermediate per-ETF-base version gave +37.49% but was rejected by user as too low.
...
|---|-------|----------|------|-------------|
| 1 | **Entry position ratio base** | Total fund ×30% | Per-ETF fund ×30% | ✅ Fixed: `_buy()` uses `target_value = self.broker.getvalue() * pct` (total fund × ratio, user chose this over per-ETF base — "应该是按比例，不要写死") |
| 2 | **Confirm add 70%** | Total fund ×70% | Per-ETF fund ×70% | ✅ Fixed: same as above — total fund × pct |
...
**After fixes**: 5yr +97.88%/8.88%dd (was +93.47%/12.92%dd). The improvement comes from adding trend profit sell + MA5 active share sell to backtest, and fixing AO calculation. User chose total fund × pct as the position ratio base (not per-ETF 20% × pct).
...
- **Backtrader multi-period backtest results (v3.3, 2026-08-01, with warmup, backtest-live consistent, total fund × pct base)**:
...
- **Backtest position ratio base — FIXED v3.3**: backtest_bt.py now uses `_buy(data, pct)` where `target_value = self.broker.getvalue() * pct` (total fund × ratio). User explicitly corrected "应该是按比例，不要写死" — do NOT hardcode per-ETF allocation or per-ETF 20% base. The ratio approach (total fund × pct) lets broker handle fund constraints naturally and matches the user's "仓位管理" concept. An intermediate per-ETF-base version gave +37.49% return but was rejected by user as too low.
- **No hardcoded absolute numbers in position sizing**: user explicitly corrected "应该是按比例，不要写死" — when computing position sizes, use `broker.getvalue() * pct` (proportional to total fund), NOT `44000 * pct` or `broker.getvalue() * 0.20 * pct` (hardcoded per-ETF allocation). The total fund changes over time, so the proportional approach is self-adjusting. Hardcoding absolute numbers creates maintenance burden and is fragile to capital changes.
- **Position ratio base choice affects backtest results dramatically**: using total fund × pct (e.g. 22万×30%=6.6万) gives +97.88% return; using per-ETF 20% × pct (4.4万×30%=1.32万) gives +37.49%. The 5x difference is because smaller positions mean less capital utilization. User chose total fund × pct. When changing position ratio base, ALWAYS re-run multi-period backtest to verify the impact.
