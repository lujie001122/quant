...
|------|----------|---------|
| backtest_bt.py | ~/hermes/scripts/ | Backtrader backtest engine (v3.2, 5-ETF shared pool, 6 channels, Analyzers for Sharpe/DD/Trade) |
| backtest_5etf_1m.py | ~/hermes/scripts/ | Legacy hand-written backtest (superseded by backtest_bt.py, kept for reference) |
...
**Backtest baseline after v3.2 optimization (2026-08-01, 3 strategy changes APPLIED, backtrader engine)**: 5yr +73.85%, drawdown 13.79%, Calmar 5.36, Sharpe 0.90, win rate 48.0%, profit factor 3.50. Three changes applied after systematic optimization testing: (1) probe entry RSI>30→40, (2) trailing stop 8% no longer sets trail line (only at 15%), (3) dip-add requires RSI<40. Two other optimizations tested and REJECTED: (a) stop-level 1 reduce 50% instead of 30% — too aggressive, -2.57% return; (b) avg-cost stop 15% instead of 10% — too loose, -32.21% return. Note: backtrader uses next-day open price for execution (more realistic), hand-written backtest used same-day close, hence ~19% lower return.
