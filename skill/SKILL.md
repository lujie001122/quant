...
- Position management: user (professional quant) corrected the fund pool model: "实盘为什么资金不能共用，仓位管理你怎么考虑的？" — the correct approach is **shared pool + per-ETF position cap**. Independent pools have low capital efficiency and artificially high drawdown. The cap prevents over-allocation. Backtest must use the same model: shared 220k cash + 44k max per ETF.
