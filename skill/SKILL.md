...
**Backtest baseline after v3.2 optimization (2026-08-01, 3 strategy changes APPLIED, backtrader engine)**: 5yr +93.47%/12.92%dd/Calmar 1.09/Sharpe 1.10/43笔/48.8%wr/3.50pf (backtrader, warmup data from 2020-01-01, fromdate=2021-08-01). **Note**: backtrader results with warmup data are higher than without (+73.85%→+93.47%) because warmup data provides indicator pre-computation. Always use warmup from ~1 year before the target period for accurate results. Three changes applied after systematic optimization testing: (1) probe entry RSI>30→40, (2) trailing stop 8% no longer sets trail line (only at 15%), (3) dip-add requires RSI<40. Two other optimizations tested and REJECTED: (a) stop-level 1 reduce 50% instead of 30% — too aggressive, -2.57% return; (b) avg-cost stop 15% instead of 10% — too loose, -32.21% return. Note: backtrader uses next-day open price for execution (more realistic), hand-written backtest used same-day close, hence ~19% lower return.
...
**⚠️ BACKTEST vs LIVE CONSISTENCY AUDIT (2026-08-01)** — 6 critical inconsistencies found between backtest_bt.py and signal_generator.py. The backtest results are NOT reliable for predicting live performance until these are fixed:

| # | Issue | Backtest | Live | Severity |
|---|-------|----------|------|----------|
| 1 | **Entry position ratio base** | Total fund 22万×30%=6.6万/笔 | Per-ETF 4.4万×30%=1.32万/笔 | 🔴 P0 — backtest buys 5x more per trade |
| 2 | **Confirm add 70%** | Total fund 22万×70%=15.4万 | Per-ETF 4.4万×70%=3.08万 | 🔴 P0 — backtest adds 5x more |
| 3 | **AO calculation** | close近似median | (H+L)/2 median | 🔴 P0 — AO values differ, affects confirm-add |
| 4 | **COOLDOWN_DAYS** | 2 | 1 | 🔴 P0 — backtest more conservative |
| 5 | **Trend profit sell + MA5 active share sell** | Not in backtest | In live (2 sell signals) | 🔴 P0 — backtest overestimates returns |
| 6 | **Overfitting risk** | 515880=15/27 buys, first 3yr zero trades | — | 🟡 P1 — strategy may not generalize |

**Root cause**: backtest_bt.py uses `_buy(data, pct)` where pct is a fraction of total fund (0.30=30% of 22万), while signal_generator.py uses per-ETF fund allocation (4.4万×30%=1.32万). This is the BIGGEST inconsistency — the backtest's position sizing is fundamentally different from live.

**Fix priority**: (1) Unify entry ratio base — either backtest uses per-ETF amounts or live uses total-fund ratios. (2) Fix AO to use (H+L)/2 in backtest. (3) Unify COOLDOWN_DAYS to 2. (4) Add trend profit sell and MA5 active share sell to backtest. (5) Run parameter sensitivity analysis on magic numbers (8%/15%/10%/20%/30%/3%/35%/40%).
...
- **Backtrader custom indicator pitfalls**: (1) `self.data.close.get(size=N)` does NOT work in custom indicators — use `[self.data[-i] for i in range(N-1, -1, -1)]` instead; (2) `self.data.high`/`self.data.low` are NOT accessible in custom indicators initialized with `d.close` — only `self.data[0]` (the close line) works; (3) for multi-input indicators, use `d.close` as single input and approximate median with close; (4) `len(self.data)` gives the number of available bars, not `len(self.data.close)`; (5) **MACDStatus buffer overflow**: when computing `closes = [self.data[-i] for i in range(size-1, -1, -1)]`, if `size` is larger than the actual buffer, it throws IndexError. Fix: only fetch the minimum needed bars: `closes = [self.data[-i] for i in range(min(size, min_needed + self.p.fast) - 1, -1, -1)]`. This is critical for short-period backtests with warmup data using `fromdate`.
...
- **Backtrader multi-period backtest results (v3.2, 2026-08-01, with warmup)**:
...
- **Backtest position ratio base is the #1 consistency risk**: backtest_bt.py uses `_buy(data, 0.30, reason)` where 0.30 = 30% of total fund value (22万×30%=6.6万). signal_generator.py uses `position_ratio = "30%"` where 30% = 30% of per-ETF allocation (4.4万×30%=1.32万). This 5x difference means the backtest takes positions 5x larger than live for the same signal. Before trusting any backtest result, verify the position ratio base matches live.
