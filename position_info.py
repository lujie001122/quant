...
def _load_config():
    ...
    try:
        ...
        _DEFAULTS['min_shares'] = cfg.get('trade', {}).get('min_shares', 5000)
    except Exception:
        pass
    return _DEFAULTS
...
MIN_SHARES = _config.get('trade', {}).get('min_shares', 5000)  # 最小交易股数(对齐实盘)
...
class PositionInfo:
    ...
    def update_trailing_stop(self, price):
        ...
        if self.reached_15pct and self.peak_price > 0:
            self.trailing_stop_price = max(self.peak_price * trail_mult, self.trailing_stop_price)  # BUGFIX: 防止被更低值覆盖
