...
def t0_buy_score(rsi_5min, macd_5min_status, vol_ratio_5min):
    ...
    if rsi_5min is None:
        return 0
    if rsi_5min > _CONF.get("t0", {}).get("rsi_extreme_sell", 75):
        return 0
    if rsi_5min < _CONF.get("t0", {}).get("rsi_extreme_buy", 25):
        return 3
    if rsi_5min >= _CONF.get("t0", {}).get("rsi_buy_max", 50):
        return 0
    score = 1
...
def t0_sell_score(rsi_5min, macd_5min_status, vol_ratio_5min):
    ...
    if rsi_5min > _CONF.get("t0", {}).get("rsi_extreme_sell", 75):
        return 3
    if rsi_5min <= _CONF.get("t0", {}).get("rsi_sell_min", 50):
        return 0
    score = 1
...
class RSIMACDStrategy(BaseStrategy):
    ...
    def evaluate_entry(self, pos, t, price, realtime, positions, all_klines, code, today_str, atr_pct, defense_weak, skip_profit_filter=False):
        ...
        if pos.build_phase == 0:
            ...
            if t["macd_status"] == "金叉" and (t["rsi"] is not None and t["rsi"] <= _CONF.get("entry", {}).get("rsi_entry_max", 55)) and pos.can_buy_today(today_str, atr_pct):
            ...
            if pos.build_first_price > 0 and pos.add_count < _CONF.get("entry", {}).get("add_count_limit", 5):
                dip_pct = (price - pos.build_first_price) / pos.build_first_price
                if dip_pct < _CONF.get("entry", {}).get("dip_pct", -0.03) and pos.can_buy_today(today_str, atr_pct) and t["macd_status"] not in ["死叉", "绿柱放大"] and (t["rsi"] is None or t["rsi"] < 40):
                    if _position_capped:
